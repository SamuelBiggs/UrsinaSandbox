...
class Player(
    Entity
):  # inherits Entity, out base class for most things. Like GameObject in Unity
    def __init__(self):
        super().__init__()
        ...
        self.origin = (-0.5, -0.5)  # sets model offset. often used when working with ui
        ...
        self.text_entity = Text(text="< a     d >", parent=scene, z=-0.1)
...
def main():
    ...
    button = Button(position=(-0.5, 0.25), text="button_text")
    button.scale *= 0.2
    button.text_entity.scale /= 0.2
    ...
    draggable = Draggable(
        scale=(0.2, 0.1),
        texture="white_cube",
        color=color.tint(color.lime, -0.5),
        text="drag & drop!",
        # world_parent = scene
    )
